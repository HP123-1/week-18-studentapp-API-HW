...
public class AssertionExampleDemo {
    ...
    static ValidatableResponse response;//variable for response
    ...
    @Test
    public void test003() {
        response.body("data.name", hasItem("ABCd"));
    }
...
//homework
    public void test004() {
        response.body("data.name", hasItem("ABCd"));//I have all products with same name = ABCd
        //response.body("data.name",hasItems("nergizer - MAX Batteries AA (4-Pack)","Duracell - 9V Batteries (2-Pack)"));//code for get list of multiple products
    }
    ...
    @Test
    public void test006() {
        response.body("data.findAll{it.name=='ABCd'}", hasItem(hasEntry("manufacturer", "Energizer")));
    ...
    public void test007() {
        response.body("limit", equalTo(10))
                .body("data.findAll{it.name=='ABCd'}", hasItem(hasEntry("manufacturer", "Energizer")))
